...
//
//  NOTE ON THE "I'VE COMPLETED PAYMENT" BUTTON:
//  This button no longer calls the backend or depends on any
//  verification result. It is hardcoded to always send the user
//  straight to HomeScreen, no matter what. Automatic confirmation
//  (switching back into the app, and the 5-second background poll)
//  still checks Paystack directly and will move the user forward on
//  its own once a payment is genuinely confirmed. If the user taps
//  "Upload" again from Home before that happens, the Pricing screen's
//  existing paid-but-unclaimed check will show the "Payment Completed"
//  lock screen instead of letting them start a new purchase.
// ════════════════════════════════════════════════════════════════════
...
class _PaymentWaitingScreenState extends State<PaymentWaitingScreen>
  ...
  // payment" button anymore (see class note above).
  ...
  // Hardcoded — always sends the user to HomeScreen, regardless of
  // payment status, network state, or anything else. No verification
  // call, no conditional routing.
  void _goHome() {
    _pollTimer?.cancel();
